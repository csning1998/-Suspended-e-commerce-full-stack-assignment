...
const PRODUCT_CATEGORIES = ['Electronics', 'Clothing', 'Books', 'Toys'] as const;
const MAX_IMAGES = 5;
...
const hasChanges = computed(() => JSON.stringify(product) !== JSON.stringify(originalProduct));
...
const fetchProductDetails = async () => {
  ...
  errorMessage.value = '';
  try {
    const response: AxiosResponse<Product> = await request.get(`/products/${productId}`);
    ...
    if (isError(error)) {
      ...
      errorMessage.value = 'An unknown error occurred while loading product details';
    }
...
const saveProduct = async () => {
  ...
  errorMessage.value = '';
  try {
    const response: AxiosResponse<Product> = await request.put(`/products/${product.id}`, product);
    ...
    if (isError(error)) {
      ...
      errorMessage.value = 'An unknown error occurred while updating the product';
    }
  ...
  <div class="form-container">
    ...
    <div v-else class="product-form">
      <div class="form-header">
        ...
        <button v-if="isEditing" @click="saveProduct" :disabled="!hasChanges || isLoading">
          Save Product
        </button>
      </div>
      ...
      <div class="form-section">
        <div class="form-field">
          ...
          <input v-if="isEditing" v-model="product.name" placeholder="Enter product name" />
        </div>
      ...
      <div class="form-section">
        ...
        <div v-for="(image, index) in product.images" :key="index" class="image-item">
          <img v-if="image" :src="image" :alt="`Product Image ${index + 1}`" />
          <input
            v-if="isEditing"
            v-model="product.images[index]"
            placeholder="Image URL"
          />
          <button v-if="isEditing" @click="removeImage(index)">Remove Image</button>
        </div>
        <button v-if="isEditing && product.images.length < MAX_IMAGES" @click="addImage">
          Add Image
        </button>
      </div>
    </div>
